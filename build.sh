...
if [ "$GO_OS_ARCH" != "" ]; then
    ...
    CGO_ENABLED=0 GOOS=$OS GOARCH=$ARCH go build "-ldflags=-s -w -buildid=" -trimpath -o $SCRIPT_DIR/_build/${OUTPUT_FILE_NAME}_${GO_OS_ARCH} .
    exit $?
fi
...
for i in $OS_ARCH; do
    ...
    echo $OS $ARCH
    CGO_ENABLED=0 GOOS=$OS GOARCH=$ARCH go build "-ldflags=-s -w -buildid=" -trimpath -o $SCRIPT_DIR/_build/${OUTPUT_FILE_NAME}_$i .
done
